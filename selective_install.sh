...
install_base_deps() {
    ...
    print_info "安装基础工具..."
    apt install -y curl wget git iptables-persistent > /dev/null 2>&1
...
install_module() {
    ...
    if declare -f install > /dev/null; then
        if install; then
            ...
            write_log "$LOG_LEVEL_INFO" "模块 $module_name 安装成功 (耗时 ${duration}秒)"
            return 0
        else
...
main() {
    check_root
    install_base_deps
    install_base_deps  # 重复调用？应该是 install_docker_if_needed，修正如下
    install_docker_if_needed
    init_credentials
